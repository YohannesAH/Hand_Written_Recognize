...
# Define the upload folder and extracted digits folder
UPLOAD_FOLDER = 'upload'
EXTRACTED_DIGITS_FOLDER = 'extracted_digits'

# Ensure the upload and extracted digits folders exist
os.makedirs(UPLOAD_FOLDER, exist_ok=True)
os.makedirs(EXTRACTED_DIGITS_FOLDER, exist_ok=True)

app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['EXTRACTED_DIGITS_FOLDER'] = EXTRACTED_DIGITS_FOLDER
...
def correct_orientation(image):
    ...
    # Apply Hough Line Transform to detect lines Honestly speaking I coudnt find the real advantage of this :)
...
def split_and_extract_digits(image_path):
    try:
        ...
        # Apply Gaussian blur for noise reduction (adjust the kernel size as needed) // I dont have the skill to manage the kernel yet
        ...
        for i, contour in enumerate(contours):
            ...
            if min_area < area and aspect_ratio < max_aspect_ratio:
                ...
                # Adjust the brightness of the extracted digit, Johan said try it with light paper
                ...
                # Save the augmented digit image with a unique filename in extracted_digits folder
                timestamp = int(time.time())  # Get a unique timestamp
                output_path = os.path.join(EXTRACTED_DIGITS_FOLDER, f'digit_{timestamp}_{i + 1}.png')
...
def extracted_file(filename):
    return send_from_directory(app.config['EXTRACTED_DIGITS_FOLDER'], filename)
